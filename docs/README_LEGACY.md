...
[![Downloads](https://img.shields.io/github/downloads/GravityPoet/ChordVox/total?style=flat&color=blue)](https://github.com/GravityPoet/ChordVox/releases)
...
Current stable release: [v1.5.18](https://github.com/GravityPoet/ChordVox/releases/tag/v1.5.18)

| 系统 System | 芯片 Chip     | 格式 Format | 下载链接 Download link                                                                                   |
| ----------- | ------------- | ----------- | -------------------------------------------------------------------------------------------------------- |
| Windows     | x64           | exe         | 暂未上传                                                                                                 |
| Mac         | Apple Silicon | dmg         | [mac-arm64](https://github.com/GravityPoet/ChordVox/releases/download/v1.5.18/ChordVox-1.5.18-arm64.dmg) |
| Mac         | Intel         | dmg         | 暂未上传                                                                                                 |
| Linux       | x64           | deb         | 暂未上传                                                                                                 |
...
[![Star History Chart](https://api.star-history.com/svg?repos=GravityPoet/ChordVox&type=date&legend=top-left)](https://www.star-history.com/#GravityPoet/ChordVox&type=date&legend=top-left)
...
ChordVox includes code derived from an upstream MIT-licensed project. See `NOTICE` for the exact attribution text.
Keep `LICENSE`, `NOTICE`, and `resources/legal/OPEN_SOURCE_NOTICES.txt` in both source and release artifacts.

## Features

- ☁️ **ChordVox Cloud**: Sign in and transcribe instantly — no API keys needed, with free and Pro plans
...
# The unsigned app will be in: dist/mac-arm64/ChordVox.app (macOS)
# or dist/win-unpacked/ChordVox.exe (Windows)
# or dist/linux-unpacked/ChordVox (Linux)
```
...
- `Apple cannot verify “ChordVox.app” is free of malware`
...
ChordVox ships a native C binary for pasting text on Windows, using the Win32 `SendInput` API. This is the **primary** paste mechanism — `nircmd` and PowerShell are only used as fallbacks if the native binary fails.
...
ChordVox now supports multiple Linux package formats for maximum compatibility:
...
# - ChordVox-x.x.x-linux-x64.AppImage
# - ChordVox-x.x.x-linux-x64.deb
# - ChordVox-x.x.x-linux-x64.rpm
# - ChordVox-x.x.x-linux-x64.tar.gz
```
...
```bash
# Debian/Ubuntu
sudo apt install ./dist/ChordVox-*-linux-x64.deb

# Fedora/RHEL
sudo dnf install ./dist/ChordVox-*-linux-x64.rpm

# Universal tar.gz (no root required)
tar -xzf dist/ChordVox-*-linux-x64.tar.gz
cd ChordVox-*/
./chordvox

# Flatpak
flatpak install --user ./dist/ChordVox-*-linux-x64.flatpak

# AppImage (existing method)
chmod +x dist/ChordVox-*.AppImage
./dist/ChordVox-*.AppImage
```

**Native Paste Binary (`linux-fast-paste`)**:

ChordVox ships a native C binary for pasting text on Linux, compiled automatically at build time. This is the **primary** paste mechanism — external tools like `xdotool` and `wtype` are only used as fallbacks if the native binary fails.
...
If the native binary isn't available, ChordVox falls back to external paste tools in this order:
...
> ℹ️ **Note**: ChordVox automatically tries paste methods in this order: native `linux-fast-paste` binary (XTest or uinput) → `wtype` → `ydotool` → `xdotool` (for XWayland apps). If no paste method works, text will still be copied to the clipboard - you'll just need to paste manually with Ctrl+V.
...
On GNOME Wayland, Electron's standard global shortcuts don't work due to Wayland's security model. ChordVox automatically uses native GNOME keyboard shortcuts via D-Bus and gsettings:

- Hotkeys are registered as GNOME custom shortcuts (visible in Settings → Keyboard → Shortcuts)
- Default hotkey is `Alt+R` on Linux, including GNOME Wayland, to avoid desktop environments reserving `Super` combinations
...
1. **Choose Processing Method**:
   - **ChordVox Cloud**: Sign in for instant cloud transcription with free and Pro plans
...
- **In-App**: Use _Settings → General → Local Model Storage → Remove Downloaded Models_ to clear `~/.cache/chordvox/whisper-models` (or `%USERPROFILE%\.cache\chordvox\whisper-models` on Windows).
...
- **macOS**: If you uninstall manually, remove `~/Library/Caches` or `~/.cache/chordvox/whisper-models` if desired.
...
- Brand names (e.g., "ChordVox", "whisper.cpp")
...
For local processing, ChordVox uses OpenAI's Whisper model via whisper.cpp - a high-performance C++ implementation:

1. **Bundled Binary**: whisper.cpp is bundled with the app for all platforms
2. **GGML Models**: Downloads optimized GGML models on first use to `~/.cache/chordvox/whisper-models/`
...
**Upgrading from Python-based version**: If you previously used the Python-based Whisper, you'll need to re-download models in GGML format. You can safely delete the old Python environment (`~/.chordvox/python/`) and PyTorch models (`~/.cache/whisper/`) to reclaim disk space.

### Local Parakeet Setup (Alternative)

ChordVox also supports NVIDIA Parakeet models via sherpa-onnx - a fast alternative to Whisper:

1. **Bundled Binary**: sherpa-onnx is bundled with the app for all platforms
2. **INT8 Quantized Models**: Efficient CPU inference
3. **Models stored in**: `~/.cache/chordvox/parakeet-models/`
...
ChordVox is designed with privacy and security in mind:
...
2. **Accessibility permissions (macOS)**: Required for automatic text pasting
   ...
   - Add ChordVox and enable the checkbox
...
- Check the [Issues](https://github.com/GravityPoet/ChordVox/issues) page
...
**Q: Is ChordVox really free?**
A: Yes! ChordVox is open source and free to use. The free plan includes 2,000 words/week of cloud transcription, and local processing is completely free with no limits. Pro plan ($9/month) offers unlimited cloud transcription.
...
A: ChordVox supports 58 languages including English, Spanish, French, German, Chinese, Japanese, and more. Set your preferred language in the .env file or use auto-detect.

## Project Status

ChordVox is actively maintained and ready for production use. Current version: 1.4.10

- ✅ Core functionality complete
- ✅ Cross-platform support (macOS, Windows, Linux)
- ✅ ChordVox Cloud with account system and usage tracking
