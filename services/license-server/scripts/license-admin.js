...
Usage:
  ...
  node scripts/license-admin.js reset-activation --key <LICENSE_KEY> [--product <PRODUCT_ID>] [--machine <MACHINE_ID>]
...
Issue options:
  ...
  --max <N>                      Ignored (single-device policy enforced: always 1)
...
Examples:
  node scripts/license-admin.js issue --email user@example.com --order ord_123
  node scripts/license-admin.js revoke --key AK-XXXX-XXXX-XXXX-XXXX --reason "Refunded"
  node scripts/license-admin.js reset-activation --key AK-XXXX-XXXX-XXXX-XXXX
  node scripts/license-admin.js reset-activation --key AK-XXXX-XXXX-XXXX-XXXX --machine <machine_id>
...
function main() {
  ...
  try {
    ...
    if (command === "reset-activation") {
      const result = store.resetActivationByKey(requireKey(options), options.product, options.machine);
      console.log(JSON.stringify(result, null, 2));
      process.exit(result.updated ? 0 : 1);
    }
