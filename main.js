...
function resolveAppChannel() {
  const rawChannel = (
    process.env.CHORDVOX_CHANNEL ||
    process.env.OPENWHISPR_CHANNEL ||
    process.env.VITE_CHORDVOX_CHANNEL ||
...
function getOAuthProtocol() {
  const fromEnv = (
    process.env.VITE_CHORDVOX_PROTOCOL ||
    process.env.CHORDVOX_PROTOCOL ||
...
function parseAuthBridgePort() {
  const raw = (
    process.env.CHORDVOX_AUTH_BRIDGE_PORT ||
...
function initializeCoreManagers() {
  ...
  windowManager.setLicenseManager(licenseManager);
...
async function startApp() {
  ...
  ipcMain.on("dictation-cue-style-changed", (_event, style) => {
    if (windowManager.mainWindow && !windowManager.mainWindow.isDestroyed()) {
      windowManager.mainWindow.webContents.send("dictation-cue-style-changed", style);
    }
  });

  ipcMain.on("recording-animation-style-changed", (_event, style) => {
    if (windowManager.mainWindow && !windowManager.mainWindow.isDestroyed()) {
      windowManager.mainWindow.webContents.send("recording-animation-style-changed", style);
    }
  });
