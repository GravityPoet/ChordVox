...
export const normalizeEndpointUrl = (value?: string | null): string => {
  if (!value) return "";
...
export const getEndpointBaseUrl = (value?: string | null): string => {
  return normalizeBaseUrl(normalizeEndpointUrl(value));
};
...
export const buildModelsUrl = (endpointOrBase: string): string => {
  const normalizedBase = getEndpointBaseUrl(endpointOrBase);
  if (!normalizedBase) return "";
  return buildApiUrl(normalizedBase, "/models");
};
...
const DEFAULT_OPENAI_BASE = computeBaseUrl(
  [
    env.CHORDVOX_OPENAI_BASE_URL as string | undefined,
...
export const API_ENDPOINTS = {
  ...
  DOUBAO_ASR_WS: "wss://openspeech.bytedance.com/api/v3/sauc/bigmodel_nostream",
...
// ChordVox Cloud API
export const CHORDVOX_API_URL =
  (env.VITE_CHORDVOX_API_URL as string) || (env.VITE_OPENWHISPR_API_URL as string) || "";
