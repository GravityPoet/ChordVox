...
interface PromptStudioProps {
  className?: string;
  ensureTestAccess?: () => Promise<boolean>;
  onUpgradeToPro?: () => void;
...
const PROVIDER_CONFIG: Record<string, ProviderConfig> = {
  ...
  [CHORDVOX_CLOUD_PROVIDER]: { label: "ChordVox Cloud" },
  custom: {
...
function migrateLegacyBrandNames(input: string): string {
  const legacyBrandNames = ["AriaKey", "Whispr", ["Open", "Whispr"].join(""), "MoonlitVoice"];
  return legacyBrandNames.reduce(
    (result, name) => result.replace(new RegExp(`\\b${name}\\b`, "gi"), "ChordVox"),
    input
  );
}
...
export default function PromptStudio({
  ...
  const [showTestRequiresPro, setShowTestRequiresPro] = useState(false);
  ...
  const testPrompt = async () => {
    ...
    if (ensureTestAccess) {
      const allowed = await ensureTestAccess();
      if (!allowed) {
        setShowTestRequiresPro(true);
        setTestResult("");
        return;
      }
    }

    setShowTestRequiresPro(false);
    setIsLoading(true);
    setTestResult("");

    try {
      const useReasoningModel = localStorage.getItem("useReasoningModel") === "true";
      const cloudReasoningMode = localStorage.getItem("cloudReasoningMode") || CHORDVOX_CLOUD_MODE;
    ...
    <div className={className}>
      ...
      <div className="rounded-xl border border-border/60 dark:border-border-subtle bg-card dark:bg-surface-2 overflow-hidden">
        ...
        {activeTab === "test" &&
          (() => {
            const useReasoningModel = localStorage.getItem("useReasoningModel") === "true";
            const cloudReasoningMode =
              localStorage.getItem("cloudReasoningMode") || CHORDVOX_CLOUD_MODE;
            const isSignedIn = localStorage.getItem("isSignedIn") === "true";
            const isCloudMode = isSignedIn && isChordVoxCloudMode(cloudReasoningMode);
                        ...
                        <div className="space-y-3">
                          <div>
                            <p className="text-[12px] font-medium text-foreground leading-relaxed">
                              {t("settingsPage.prompts.requiresProTitle")}
                            </p>
                            <p className="mt-1 text-[12px] text-muted-foreground leading-relaxed">
                              {t("settingsPage.prompts.requiresProDescription")}
                            </p>
                          </div>
                          <div className="flex items-center gap-2">
