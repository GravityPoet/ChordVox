...
function registerEventListeners() {
  ...
  if (window.electronAPI.onUpdateAvailable) {
    const dispose = window.electronAPI.onUpdateAvailable((_event, info) => {
      updateGlobalState({
        ...
        isChecking: false,
        isDownloading: info?.manualOnly !== true,
      });
    });
    if (dispose) cleanupFunctions.push(dispose);
  }
  ...
  if (window.electronAPI.onUpdateDownloaded) {
    const dispose = window.electronAPI.onUpdateDownloaded((_event, info) => {
      updateGlobalState({
        status: { ...globalState.status, updateDownloaded: true, lastCheckedAt: new Date().toISOString() },
        info: info || globalState.info,
    ...
    const initializeUpdateStatus = async () => {
      try {
        if (window.electronAPI?.getUpdateStatus) {
          ...
          updateGlobalState({
            status,
            isDownloading: Boolean(status?.isDownloading),
          });
        }
