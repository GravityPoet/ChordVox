...
// Modifier-only combos (e.g. "Control+Alt") bypass globalShortcut on Windows
...
function usesNativeHotkeyListener(hotkey, platform = process.platform) {
  if (!hotkey) return false;
  if (hotkey === "GLOBE") return true;
  if (isRightSideModifier(hotkey)) return true;
  return platform === "win32" && isModifierOnlyHotkey(hotkey);
}

function getHotkeyAccelerator(hotkey) {
  return hotkey && hotkey.startsWith("Fn+") ? hotkey.slice(3) : hotkey;
}
...
const SUGGESTED_HOTKEYS = {
  ...
  compound: ["Control+Alt", "Control+Shift+Space", "Alt+R", "Alt+F7"],
};

class HotkeyManager {
  constructor() {
    this.currentHotkey = getDefaultHotkeyForPlatform();
  ...
  getSuggestions(failedHotkey) {
    ...
    if (process.platform === "darwin" && isCompound) {
      ...
      suggestions = ["F8", "F9", "Control+Shift+K"];
  ...
  setupShortcuts(hotkey = getDefaultHotkeyForPlatform(), callback) {
    if (!callback) {
    ...
    const checkAccelerator = getHotkeyAccelerator(hotkey);
    if (
      hotkey === this.currentHotkey &&
      !usesNativeHotkeyListener(hotkey) &&
    ...
    if (this.currentHotkey && !usesNativeHotkeyListener(this.currentHotkey)) {
      const prevAccelerator = getHotkeyAccelerator(this.currentHotkey);
      try {
    ...
    try {
      ...
      const accelerator = getHotkeyAccelerator(hotkey);
  ...
  _restorePreviousHotkey(previousHotkey, callback) {
    if (!previousHotkey || usesNativeHotkeyListener(previousHotkey)) {
      return;
    }
    const prevAccel = getHotkeyAccelerator(previousHotkey);
    try {
  ...
  async loadSavedHotkeyOrDefault(mainWindow, callback) {
    try {
      ...
      const defaultHotkey = getDefaultHotkeyForPlatform();
...
module.exports.usesNativeHotkeyListener = usesNativeHotkeyListener;
module.exports.getHotkeyAccelerator = getHotkeyAccelerator;
module.exports.getDefaultHotkeyForPlatform = getDefaultHotkeyForPlatform;
