...
let debugLogger = fallbackLogger;
try {
  debugLogger = require("./debugLogger");
} catch {
  debugLogger = fallbackLogger;
}

const USER_AGENT = "ChordVox/1.0";
      ...
      if (statusCode >= 300 && statusCode < 400) {
        ...
        try {
          ...
          reject(
            Object.assign(
              new Error(
                `Invalid redirect URL: ${error instanceof Error ? error.message : String(error)}`
              ),
              { code: "ERR_INVALID_REDIRECT_URL" }
            )
          );
          return;
        }
        debugLogger.info("Following download redirect", {
          from: url.substring(0, 80),
          to: redirectUrl.substring(0, 80),
        });
        downloadAttempt(redirectUrl, tempPath, {
          ...options,
