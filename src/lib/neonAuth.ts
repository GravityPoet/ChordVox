...
import {
  CHORDVOX_LAST_SIGN_IN_STORAGE_KEY,
  LEGACY_CLOUD_LAST_SIGN_IN_STORAGE_KEY,
  getStorageValueWithFallback,
...
function loadLastSignInTimeFromStorage(): number | null {
  ...
  const raw = getStorageValueWithFallback(storage, [
    CHORDVOX_LAST_SIGN_IN_STORAGE_KEY,
    LEGACY_CLOUD_LAST_SIGN_IN_STORAGE_KEY,
  ]);
  if (!raw) return null;
  ...
  if (!Number.isFinite(parsed) || parsed <= 0) {
    storage.removeItem(CHORDVOX_LAST_SIGN_IN_STORAGE_KEY);
    storage.removeItem(LEGACY_CLOUD_LAST_SIGN_IN_STORAGE_KEY);
    return null;
  }
...
function persistLastSignInTime(value: number | null): void {
  ...
  if (value === null) {
    ...
    storage.setItem(CHORDVOX_LAST_SIGN_IN_STORAGE_KEY, String(value));
    storage.removeItem(LEGACY_CLOUD_LAST_SIGN_IN_STORAGE_KEY);
  }
}
...
function getElectronOAuthCallbackURL(): string {
  const configuredUrl =
    (import.meta.env.VITE_CHORDVOX_OAUTH_CALLBACK_URL ||
      import.meta.env.VITE_OPENWHISPR_OAUTH_CALLBACK_URL ||
      "").trim();
...
export async function requestPasswordReset(email: string): Promise<{ error?: Error }> {
  ...
  try {
    if (CHORDVOX_API_URL) {
      const res = await fetch(`${CHORDVOX_API_URL}/api/auth/forgot-password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
