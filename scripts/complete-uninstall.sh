...
echo "This script will remove ChordVox and any legacy app data paths, then delete caches, databases, and preferences."
...
echo "Stopping running ChordVox and legacy Electron processes..."
pkill -f "OpenWhispr" 2>/dev/null || true
pkill -f "open-whispr" 2>/dev/null || true
pkill -f "Electron Helper.*OpenWhispr" 2>/dev/null || true

echo "Removing installed app bundles (current and legacy names, requires admin)..."
remove_target "/Applications/OpenWhispr.app"

echo "Purging Application Support data (including legacy paths)..."
...
echo "Removing caches, logs, and saved state (including legacy paths)..."
